...
export default function SemanticSearchPage() {
  ...
  async function handleSearch(event: React.FormEvent) {
    ...
    try {
      const response = (await apiRequest(`/search/semantic?q=${encodeURIComponent(query.trim())}`)) as {
        results: Result[];
      };
    ...
    const localMatches = searchCatalog(query.trim()).map((m) => ({
      id: m.id,
      title: m.title,
      overview: m.overview,
      ...
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        <form onSubmit={handleSearch} className="glass-panel" style={{ display: 'flex', padding: 8, gap: 8 }}>
          ...
          <input
            ...
            style={{ flex: 1 }}
          />
          <button type="button" aria-label="Voice search is not available" disabled title="Voice search requires browser speech recognition">
            <Mic size={22} />
          </button>
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              {results.map((movie) => (
                <Link
                  ...
                  style={{ display: 'flex', padding: 16, gap: 20 }}
                >
                  {movie.poster_path && (
                    <Image
